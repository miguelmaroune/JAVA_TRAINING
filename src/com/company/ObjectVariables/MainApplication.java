package com.company.ObjectVariables;
...
public class MainApplication {
    ...
    public static void mainAppMatrix() throws IOException {
...
//This function will be called by the MatricesReaderProducer and add the pair of matrics from the file.
//The read from the file is much more faster the  job the consumer needs to do so to avoid having a very long queue
//we set a capacity for our queue (5) , whenever the MatricesReaderProducer is trying to insert another pair and the capacity is reached -> wait.
// The wait will force the thread to stop until another thread notifies it to continue
        public synchronized void add(MatricesPair matricesPair) {
            ...
            queue.add(matricesPair);
            isEmpty = false;
            //After adding a pair to the queue notify the consumer Thread in order to continue its work.
            notify();
        }
//The remove is called from the MatricesMultiplierConsumer thread which will first remove the Matrics pairs from the queue ,
//do the calculation and finally write the result in the new file (this process will take a lot more time than the first one )
        public synchronized MatricesPair remove() {
            MatricesPair matricesPair = null;
            while (isEmpty && !isTerminate) {
                try {//If there are no pairs in the queue wait .
                    wait();
                } catch (InterruptedException e) {
                }
            }
            ...
            //After removing a pair from the queue if the queue size is less than the capacity , Notify the producer thread that it can continue to adding another pair
